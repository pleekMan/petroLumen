...
for i in range(100):
    nodesX.append(random.random() * window.width)
    nodesY.append(random.random() * window.height)
...
def on_draw():
    ...
    for i in range(len(nodesX)):
        localTimeX = mapToRange(nodesX[i],0,window.width,timeX, timeX + timeFarLimit)
        localTimeY = mapToRange(nodesY[i],0,window.height,timeY, timeY + timeFarLimit)
